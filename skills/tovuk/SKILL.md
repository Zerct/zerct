...
5. For a new full-stack project, run `npx tovuk init my-app --template fullstack-rust-tanstack`.
6. Run `npx tovuk doctor --json`.
7. Run `npx tovuk preview api` and `npx tovuk preview web` when local tools are available.
8. Run `npx tovuk deploy --wait --json`. From a repo root with nested `tovuk.toml` files, this deploys the whole workspace in one command.
9. If Tovuk returns an `agent_instruction`, apply it, rerun doctor, and redeploy.
10. If a build fails, run `npx tovuk logs --build <build_id> --json`, fix the first actionable log error, rerun doctor, and redeploy.
