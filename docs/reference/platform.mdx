...
resource fits a task, use each product's `meters` to set hard caps, and use the
plan `limits` and `pricing` objects to estimate free and paid usage before
deployment. The authenticated usage response adds `billingEstimate.lineItems`
for current-month cost planning.
...
capped at 2 MiB combined. Free State objects get 1 GB per object and a 5 GB
