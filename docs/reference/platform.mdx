...
resource fits a task, use each product's `meters` and `meter_details` to set
hard caps with the right units, use `pricing_fields` to read the relevant plan
prices, and use the plan `limits` and `pricing` objects to estimate free and
paid usage before deployment. The authenticated usage response adds
