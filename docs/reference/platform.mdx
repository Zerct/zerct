...
Use the `products` array to decide which resource fits a task, the `meters`
array to set hard caps, and the plan `limits` and `pricing` objects to estimate
free and paid usage before deployment.
