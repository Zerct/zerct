...
description: "Confirm account setup, then create, inspect, and delete Tovuk service resources with CLI, API, and dashboard workflows."
---
...
<Steps>
  <Step title="Confirm account and Service context">
    ...
    Confirm account setup first, then use the Service snapshot to review
    current resources, capabilities, `accountUsage`, `billingEstimate`, and
    top-level `next_actions` before creating more resources.
  </Step>

  <Step title="Create the required resource">
    ```sh
    tovuk sqlite create --service service_1 DB --json
    tovuk kv create --service service_1 CACHE --json
    ```

    Use the resource-specific CLI or API route for the capability your worker
    needs. The dashboard uses the same routes and limits.
  </Step>

  <Step title="Verify the refreshed resources">
    ```sh
    tovuk service show service_1 --json
    ```

    Confirm the new resource appears under `resources`, then review top-level
    `next_actions` and nested `resources.nextActions` for follow-up commands.
  </Step>

  <Step title="Deploy and inspect resources">
    ```sh
    tovuk deploy --wait --json
    tovuk service show <service> --json
    ```

    Deploy after required service resources exist, then use the post-deploy
    Service snapshot to confirm runtime bindings, resource counts,
    `accountUsage`, `billingEstimate`, and next actions. If the deploy fails,
    switch to Logs and Builds before changing resource setup.
  </Step>
</Steps>
...
or `https://tovuk.com/<handle>/resources`. It uses CLI-equivalent routes and
limits for resource work. Dashboard resource actions use the same API routes and limits as the CLI.

- Create, list, and delete SQLite databases, KV namespaces, queues, Cron
  triggers, State namespaces, and Service Binding resources through the same API routes as the CLI. This includes creating and deleting SQLite databases
  and the other service resources.
- Run single SQLite queries and transaction batches through
  `POST /v1/services/{service_id}/sqlite/{database}/query` and inspect rows,
  `rowsRead`, `rowsWritten`, `vmSteps`, `fullscanSteps`, storage, duration,
  and next actions.
- Use the dashboard to list SQLite backups, create SQLite backups, and restore SQLite backups.
- Use the dashboard to list KV keys, read one KV key, write one KV key, and delete one KV key
  through the same metered KV API routes as the CLI.
- Send one Queue message, send one Queue batch, read Queue messages, read Queue
  metrics, and update Queue retries, retention, batch size, batch timeout, and
  dead-letter Queue settings through
  `PUT /v1/services/{service_id}/queues/{queue}`.
- Use the dashboard to update Cron schedules, pause Cron triggers, and resume
  Cron triggers.
- List State objects, list State keys, read one State value, write one State
  value, delete one State value, read State alarms, set State alarms, and delete
  State alarms through the same metered State API routes as the CLI.

Use CLI bulk commands for bulk KV reads, writes, and removals.
...
- Secrets: server-only environment variables managed with `tovuk secrets`.
...
`tovuk service status <service> --json` is the compact post-deploy status check
for agents. It returns top-level `ok`, `url`, and `agent_instruction` fields,
plus runtime status, latest deploy, latest build, and a `live` boolean.
`tovuk service show <service> --json` is the full service snapshot for agents.
It includes status, deploys, builds, logs, env names, domains, resources,
capabilities, `accountUsage`, `billingEstimate`, and next actions in one
response.
