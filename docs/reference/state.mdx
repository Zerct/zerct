---
title: "Manage State Objects and Alarms"
sidebarTitle: "State"
description: "Confirm account and Service context, then manage keyed State objects, alarms, runtime bindings, and coordination for Tovuk Services."
---

State classes are Service-scoped keyed objects accessed from the Rust Worker
runtime. Use them when one object key needs serialized coordination, realtime
session data, counters, locks, rooms, or per-tenant state.

Keep public HTTP APIs in the Rust worker runtime. Call State from that worker
only when the request needs keyed coordination or per-object alarm scheduling.
