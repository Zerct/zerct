...
`github.com` blob URLs, or public `raw.githubusercontent.com` file URLs, and
...
URLs, `postId`, and public usernames. Use `contentType: "posts"`,
`contentType: "comments"`, or `contentType: "both"` to select post search,
comment search, or both result types in one Request. Set
...
`subreddit`, `search`, `search-comments`, `search-communities`, `search-users`,
`subreddit-users`, `post`, `comments`, `user`, `user-posts`, `user-comments`,
`user-overview`, `popular`, and `all`. For direct post or comment requests,
`commentSort` aliases `sort` and supports public Reddit comment ordering.
...
internally and only accepts public Reddit targets and public search filters.
