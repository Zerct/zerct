...
`operation: "trendingDevelopers"`, and `operation: "opportunities"`. It also
accepts `mode: "developers"` as an actor-compatible alias for trending
developers, plus `operation: "marketplace"` with `query` or `searchQuery` for
Marketplace search, `marketplaceUrls`, and Apify-compatible `appsUrls` for
public Marketplace app pages. Tovuk stores canonical public input fields and
returns repository-shaped, developer-shaped, or Marketplace app Records.
...
runs reads through managed managed proxy provider egress.
