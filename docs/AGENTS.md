...
4. Docs are dark-only. Keep `appearance.default` set to `dark`,
   `appearance.strict` set to `true`, and the navbar logo on the white Zerct
   text logo.
5. Use sentence case headings, short paragraphs, and concrete commands.
6. Keep examples copy-pasteable and agent-readable.
7. Do not document unlaunched product behavior as available. Mark it as planned
   or omit it.
8. Do not commit secrets, API keys, customer data, private URLs, screenshots
...
10. OpenAPI changes must keep `docs/openapi.json` valid and pass
   ...
   `100/100`.
11. After editing navigation, run `node scripts/check-docs.mjs` from the repo
    root.
12. Before finishing docs work, run `./scripts/check-all.sh` from the repo root.
