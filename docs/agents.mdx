...
8. If a newer deploy supersedes queued work, run `tovuk deploy cancel <deploy_id> --json`.
9. If the build fails, run `tovuk logs --build <build_id> --json`.
10. Fix the first actionable error from the logs.
11. Redeploy with the same deploy command.
12. For service files or media, use `tovuk storage ... --json` instead of the
   dashboard.
13. For background work, create a queue and implement
   `POST /.tovuk/queues/<queue>` in the Rust worker.
14. For scheduled work, create a cron trigger and implement
   `POST /.tovuk/cron/<trigger>` in the Rust worker.
15. Before high-throughput work, run `tovuk pricing --json`, then
...
`tovuk deploy --wait --json`. Cancel superseded queued work with
