...
4. Run `tovuk deploy --dry-run --json`.
...
8. Inspect deploy state with `tovuk deploy list --json` or
   `tovuk deploy show <deploy_id> --json` when the deploy result is not enough.
9. If a newer deploy supersedes queued work, run `tovuk deploy cancel <deploy_id> --json`.
10. If the build fails, run `tovuk logs --build <build_id> --json`.
11. Fix the first actionable error from the logs.
12. Redeploy with the same deploy command.
13. For service files or media, use `tovuk storage ... --json` instead of the
   dashboard.
14. For background work, create a queue and implement
   `POST /.tovuk/queues/<queue>` in the Rust worker.
15. For scheduled work, create a cron trigger and implement
   `POST /.tovuk/cron/<trigger>` in the Rust worker.
16. Before high-throughput work, run `tovuk pricing --json`, then
   `tovuk usage --json` and inspect `billingEstimate.lineItems`.
   Set usage caps for the relevant meters.
17. If a plan limit blocks the deploy or storage action, run
   `tovuk billing checkout --json` and show the returned
   `checkout.url` to the human.
18. If Tovuk itself needs help, create a support ticket with the exact command,
   service id, build id, deploy id, and first actionable log line.
19. Resolve the support ticket after the agent fixes the issue.
20. Delete throwaway smoke-test services with
