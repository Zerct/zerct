...
const FIXTURE_FILES = ['install-policy.mjs', 'install.mjs', 'native-release-targets.json', 'package.json']
const HELLO_SHA256 = '2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824'
const FIRST_REQUEST_COUNT = 1
const INITIAL_REDIRECT_COUNT = 0
const TEST_MAX_BYTES = 5
const SHA256_HEX_LENGTH = 64
const { assertAllowedDownloadUrl, assertDownloadSize, fetchResponse, parseChecksum, verifySha256, writeResponse } =
    installer

const untrustedFetch = () => new Response(null, { headers: { location: 'https://example.com/native' }, status: 302 })

const loopingFetch = () => new Response(null, { headers: { location: 'https://github.com/again' }, status: 302 })
...
const assertStreamedSizes = async (fixture) => {
    ...
    await assert.rejects(writeResponse(emptyResponse, emptyPath, TEST_MAX_BYTES), /response is empty/u)
...
const createFixture = async (context) => {
    ...
    await Promise.all(FIXTURE_FILES.map((file) => copyFile(join(sourceRoot, file), join(fixture, file))))
    return fixture
}
...
test('reports the stable JSON error contract when the native binary is absent', async (context) => {
    const fixture = await createFixture(context)
    await assert.rejects(execFileAsync(process.execPath, [join(fixture, 'bin', 'tovuk.mjs'), '--json']), (error) => {
...
test('validates every redirect and caps the redirect chain', async () => {
    ...
    const response = await fetchResponse('https://github.com/release', INITIAL_REDIRECT_COUNT, trustedFetch)
    assert.equal(await response.text(), 'native')
    assert.deepEqual(requests, ['https://github.com/release', 'https://objects.githubusercontent.com/native'])
...
test('binds a single checksum line to the requested asset', () => {
    ...
    assert.throws(() => parseChecksum(`${HELLO_SHA256}  different\n`, 'tovuk-native'), /names different/u)
    assert.throws(() => parseChecksum(`${HELLO_SHA256}\n${HELLO_SHA256}\n`, 'tovuk-native'), /exactly one/u)
