...
async function deploy(projectDir: string, cli: CliOptions): Promise<void> {
  ...
  if (projects.length === 0) {
    throw agentError('missing_project_contract', 'No tovuk.toml was found.', 'Run `tovuk init` in each app directory, or pass a project path.', cli.json)
  }
...
function printDeployResult(response: DeployResponse, cli: CliOptions): void {
  ...
  console.log(`next tovuk logs --app ${response.app.id}`)
}
...
function printWorkspaceDeployResults(projectDir: string, results: WorkspaceDeployResult[], cli: CliOptions): void {
  ...
  if (firstApp) {
    console.log(`next tovuk logs --app ${firstApp}`)
  }
}
...
async function waitForBuild(cli: CliOptions, token: string, buildId: string): Promise<BuildRecord> {
  ...
  while (Date.now() <= deadline) {
    ...
    if (!build?.status) {
      throw agentError('build_status_unavailable', 'Build status is unavailable.', `Retry with \`tovuk logs --build ${buildId}\`.`, cli.json)
    }
  ...
  throw agentError(
    ...
    `Run \`tovuk logs --build ${buildId}\` to continue watching.`,
    cli.json
  )
}
