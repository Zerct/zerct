...
async function loginAndStore(cli: CliOptions): Promise<string> {
  ...
  if (!start.loginUrl) {
    throw agentError('login_failed', 'Tovuk login did not return a browser URL.', 'Retry `tovuk login`. If it keeps failing, check Tovuk status.', cli.json)
  }
  ...
  if (!session.token) {
    throw agentError('login_failed', 'Tovuk login did not return a session token.', 'Run `tovuk login` again and complete the browser login.', cli.json)
  }
...
async function pollLogin(cli: CliOptions, start: LoginStartResponse): Promise<LoginPollResponse> {
  if (!start.deviceCode) {
    throw agentError('login_failed', 'Tovuk login did not return a device code.', 'Retry `tovuk login`. If it keeps failing, check Tovuk status.', cli.json)
  }
  ...
  while (Date.now() < deadline) {
    ...
    if (response.status === 'expired') {
      throw agentError('login_expired', 'Tovuk login expired before it completed.', 'Run `tovuk login` again and finish the browser login in the newly opened tab.', cli.json)
    }
    intervalMs = Math.max(
...
function writeSessionToken(token: string): void {
  ...
  if (!cleanToken) {
    throw agentError('login_failed', 'Tovuk session token is empty.', 'Run `tovuk login` again and complete the browser login.', false)
  }
