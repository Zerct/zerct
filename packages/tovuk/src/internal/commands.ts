...
async function logs(cli: CliOptions): Promise<void> {
  ...
  if (response.has_more && response.next_cursor) {
    console.log(`next tovuk logs ${request.target} --cursor ${response.next_cursor}`)
  }
}
...
async function envCommand(cli: CliOptions): Promise<void> {
  await runSubcommand(cli, ENV_COMMANDS, 'list', 'Unknown env command.', 'Use `tovuk env list`, `env set`, or `env delete`.')
}

async function envDelete(cli: CliOptions): Promise<void> {
  const name = requireCommandArg(cli, 'invalid_env', 'Environment variable name is required.', 'Use `tovuk env delete --app <app> KEY`.')
...
function domainMutation(method: DomainMethod, route: DomainRoute, body: DomainBody): SubcommandHandler {
  return async (cli): Promise<void> => {
    const domain = requireCommandArg(cli, 'missing_domain', 'Domain is required.', 'Use `tovuk domains add --app <app> api.example.com`.')
    const app = requireApp(cli)
...
function billingAction(value: string, json: boolean): BillingAction {
  ...
  throw agentError('unknown_billing_command', 'Unknown billing command.', 'Use `tovuk billing checkout --json` or `tovuk billing portal`.', json)
}

async function support(cli: CliOptions): Promise<void> {
  await runSubcommand(cli, SUPPORT_COMMANDS, 'list', 'Unknown support command.', 'Use `tovuk support list --json` or `support create` with subject and details.')
}
