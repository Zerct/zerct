use super::{
    api_commands::{
        app_get_command, app_route, billing_command, binding_command, builds_command, capabilities,
        caps_command, cron_command, deploys_command, domains_command, durable_command, env_command,
        kv_command, logs_command, platform_command, print_authenticated, print_paged_authenticated,
        queue_command, sqlite_command, storage_command, support_command,
    },
...
pub(crate) fn run() -> Result<ExitCode> {
    ...
    match cli.command.as_str() {
        ...
        "db" | "database" | "sqlite" => sqlite_command(&cli),
        "platform" => platform_command(&cli),
        "kv" => kv_command(&cli),
        "queue" | "queues" => queue_command(&cli),
        "cron" => cron_command(&cli),
        "durable" | "durable-object" | "durable-objects" => durable_command(&cli),
        "binding" | "bindings" => binding_command(&cli),
        "caps" => caps_command(&cli),
        "env" => env_command(&cli),
