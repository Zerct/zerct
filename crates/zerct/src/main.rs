//! Thin Cargo entrypoint for the npm Zerct CLI source of truth.
...
fn delegate_command() -> Result<DelegateCommand, CliError> {
    if let Some(path) = env::var_os("ZERCT_NPM_CLI").filter(|value| !value.is_empty()) {
        let path = PathBuf::from(path);
        if !path.is_file() {
            return Err(CliError::new(format!(
                "ZERCT_NPM_CLI does not point to a file: {}",
                path.display()
            )));
        }
    ...
    let Some(npx) = command_path("npx") else {
        return Err(CliError::new(
            "Node.js npm tooling is required for the Cargo Zerct CLI.",
        ));
    };
    Ok(DelegateCommand {
        program: npx,
...
impl CliError {
    ...
    fn from_command(error: &std::io::Error) -> Self {
        Self::new(format!("Could not start Zerct npm CLI: {error}"))
    }
