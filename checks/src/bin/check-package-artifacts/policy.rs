...
use tovuk_public_checks::check_support::{CheckResult, reject_secret_signatures};
...
const _: [usize; 0x000e] = [
    size_of_val(&is_packaged_cargo_config),
...
/// Return whether a member is the one publishable Cargo configuration path.
fn is_packaged_cargo_config(path: &str) -> bool {
    let mut components = path.split('/');
    let Some(root) = components.next() else {
        return false;
    };
    return root.starts_with("tovuk-")
        && components.next() == Some(".cargo")
        && components.next() == Some("config.toml")
...
pub(super) fn reject_sensitive_content(path: &str, contents: &[u8]) -> CheckResult {
    return reject_secret_signatures(format!("package member {path}").as_str(), contents);
}
