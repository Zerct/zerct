...
checks_manifest="crates/tovuk-public-checks/Cargo.toml"
cargo fmt --check --manifest-path "$checks_manifest"
cargo check --locked --release --all-targets --all-features --manifest-path "$checks_manifest"
cargo test --locked --release --all-targets --all-features --manifest-path "$checks_manifest"
cargo clippy --manifest-path "$checks_manifest" "${strict_clippy_args[@]}"
mkdir -p target
