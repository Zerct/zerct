...
const binPath = path.join(packageDir, 'src', 'tovuk.ts')
...
const tsconfig = assertObject(readJson(tsconfigPath, 'packages/tovuk/tsconfig.json'), 'tsconfig.json')
...
if (javascriptFiles.length > 0) {
  fail(`packages/tovuk must stay TypeScript-only: ${javascriptFiles.map(relativeToPackage).join(', ')}`)
}
...
assertEqual(assertObject(packageJson.bin, 'package bin').tovuk, 'src/tovuk.ts', 'tovuk bin path')
...
if (!binSource.startsWith('#!/usr/bin/env tsx\n')) {
  fail('src/tovuk.ts must keep the tsx shebang')
}
if ((statSync(binPath).mode & 0o111) === 0) {
  fail('src/tovuk.ts must stay executable')
}
...
console.log('Checked npm CLI package policy.')
