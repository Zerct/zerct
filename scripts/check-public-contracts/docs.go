...
func checkDocs() {
	...
	requireContains(limits, "State SQLite storage is 10 GB per object on Free and Pro", "State Free storage limit docs")
	...
	requireContains(limits, "4,768,371 MiB (`5 TB`) object storage object size ceiling", "object storage max object docs")
	requireContains(limits, "4,768 MiB", "object storage single-part docs")
	...
	requireContains(storage, "4,768,371 MiB (`5 TB`) object storage maximum object size", "object storage provider max docs")
	requireContains(storage, "4,768 MiB (`5 GB`)", "object storage provider single-part docs")
	requireContains(platform, "Free and Pro State objects get 10 GB per object", "platform State Free storage docs")
	...
	requireContains(pricing, "Pro costs `$25/month`", "pricing Pro subscription docs")
	...
	requireContains(openapi, `"subscriptionUsdCentsPerMonth": 2500`, "OpenAPI Pro subscription price")
	requireContains(openapi, `"subscriptionUsdMicrosPerMonth": 25000000`, "OpenAPI Pro subscription estimate")
	...
	requireContains(openapi, `"stateSqliteStorageMibPerObject": 9536`, "OpenAPI State storage limit")
	...
	requireContains(openapi, `"storageObjectMaxMib": 4768371`, "OpenAPI object size limit")
	requireContains(openapi, `"storageSinglePartUploadMaxMib": 4768`, "OpenAPI single-part upload limit")
	requireContains(openapi, `"storageObjectMaxMib": 4768371`, "OpenAPI storage object size limit")
	...
	rejectContains(limits, "same 5 TiB object ceiling", "stale object storage object docs")
	rejectContains(pricing, "`5 TiB` maximum object size", "stale object storage pricing docs")
	rejectContains(limits, "4.995 TiB", "stale object storage object docs")
	rejectContains(limits, "5115 MiB", "stale object storage single-part docs")
	rejectContains(limits, "5120 MiB", "stale object storage single-part docs")
	rejectContains(storage, "4.995 TiB", "stale object storage object docs")
	rejectContains(storage, "5115 MiB", "stale object storage single-part docs")
	rejectContains(limits, "when multipart upload support is exposed", "stale multipart docs")
	rejectContains(platform, "Free State objects get 1 GB per object", "stale platform State storage docs")
	rejectContains(pricing, "$5/month", "stale Pro subscription docs")
	rejectContains(openapi, `"subscriptionUsdCentsPerMonth": 500`, "stale OpenAPI Pro subscription price")
	rejectContains(openapi, `"subscriptionUsdMicrosPerMonth": 5000000`, "stale OpenAPI Pro subscription estimate")
