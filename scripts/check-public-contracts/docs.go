...
func checkDocs() {
	...
	requireContains(openapi, `"storageMinimumBillableObjectBytes": 64000`, "OpenAPI storage minimum billable object limit")
	requireContains(openapi, `"storageObjectsPerBucket": 50000000`, "OpenAPI storage bucket object limit")
	requireContains(openapi, `"storageRequestsPerSecondPerBucket": 750`, "OpenAPI storage bucket request throughput limit")
	...
	rejectContains(pricing, "`10 GB` per object, and `2 MiB`", "stale Free State storage docs")
	rejectContains(limits, "State SQLite storage is 10 GB per object on Free", "stale State storage docs")
	...
	rejectContains(platform, "Free State objects get 10 GB per object", "stale platform State storage docs")
