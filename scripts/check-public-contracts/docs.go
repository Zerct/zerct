...
func checkDocs() {
	...
	requireContains(storage, "object_storage = true", "object storage capability docs")
	requireContains(storage, "tovuk deploy --dry-run --json", "object storage dry-run docs")
	...
	requireContains(llms, "explicit `object_storage` capability", "llms object storage capability guidance")
	requireContains(llms, "`tovuk deploy --dry-run --json`", "llms object storage dry-run guidance")
	...
	requireContains(sqlite, "sqlite = true", "SQLite capability docs")
	requireContains(sqlite, "tovuk deploy --dry-run --json", "SQLite dry-run docs")
	requireContains(llms, "explicit `sqlite` capability", "llms SQLite capability guidance")
	requireContains(llms, "Before SQLite mutations", "llms SQLite dry-run guidance")
	...
	requireContains(state, "state = true", "State capability docs")
	requireContains(state, "tovuk deploy --dry-run --json", "State dry-run docs")
	...
	requireContains(llms, "explicit `state` capability", "llms State capability guidance")
	requireContains(llms, "Before State mutations", "llms State dry-run guidance")
	requireContains(bindings, "/v1/services/{service_id}/service-bindings", "service binding API docs")
	requireContains(bindings, "service_bindings = true", "service binding capability docs")
	requireContains(bindings, "tovuk deploy --dry-run --json", "service binding dry-run docs")
	requireContains(llms, "explicit `service_bindings` capability", "llms service binding capability guidance")
	requireContains(llms, "Before Service Binding mutations", "llms service binding dry-run guidance")
	...
	requireContains(usageCaps, "`PUT /v1/usage/caps/{metric}` sets or replaces one cap", "usage cap API set route docs")
	requireContains(usageCaps, "`period`, `capValue`, `hardStop`, and `notifyAtPercent`", "usage cap API request body docs")
	requireContains(usageCaps, "`hardStop: true`", "usage cap CLI hard stop body docs")
	requireContains(usageCaps, "`DELETE /v1/usage/caps/{metric}/{period}` removes one cap", "usage cap API delete route docs")
	...
	requireContains(llms, "`PUT /v1/usage/caps/{metric}`", "llms usage cap API set route guidance")
	requireContains(llms, "`period`,\n`capValue`, `hardStop`, and `notifyAtPercent`", "llms usage cap API request body guidance")
	requireContains(llms, "`hardStop: true`", "llms usage cap CLI hard stop guidance")
	...
	requireContains(kv, "kv = true", "KV capability docs")
	requireContains(kv, "tovuk deploy --dry-run --json", "KV dry-run docs")
	requireContains(llms, "explicit `kv` capability", "llms KV capability guidance")
	requireContains(llms, "Before KV mutations", "llms KV dry-run guidance")
	...
	requireContains(queues, "queue = true", "queue capability docs")
	requireContains(queues, "tovuk deploy --dry-run --json", "queue dry-run docs")
	requireContains(llms, "explicit `queue` capability", "llms queue capability guidance")
	requireContains(llms, "Before Queue mutations", "llms queue dry-run guidance")
	...
	requireContains(cron, "cron = true", "cron capability docs")
	requireContains(cron, "tovuk deploy --dry-run --json", "cron dry-run docs")
	...
	requireContains(llms, "explicit `cron` capability", "llms cron capability guidance")
	requireContains(llms, "Before Cron trigger mutations", "llms cron dry-run guidance")
	...
	requireContains(secrets, "tovuk secrets set", "secret CLI alias docs")
	...
	requireContains(llms, "tovuk secrets set", "llms secret CLI alias guidance")
