...
func checkDocs() {
	...
	requireContains(resources, "Use CLI bulk commands for bulk KV reads", "dashboard KV bulk guidance")
	requireContains(resources, "list State objects, list State keys", "dashboard State guidance")
	...
	requireContains(llms, "list State objects, list State keys", "llms dashboard State guidance")
	...
	requireContains(openapi, `"tovuk limits set worker_requests --period day --value <value> --json"`, "OpenAPI meter daily cap command")
	...
	requireContains(limits, "tovuk limits set object_storage_class_a_operations --period month --value 1000000 --json", "limit docs object storage Class A cap command")
	requireContains(limits, "tovuk limits set object_storage_class_b_operations --period month --value 10000000 --json", "limit docs object storage Class B cap command")
	...
	requireContains(llms, "above 100 MiB", "llms dashboard storage multipart guidance")
	...
	requireContains(logsBuilds, "same activity and log API routes", "dashboard activity API parity docs")
